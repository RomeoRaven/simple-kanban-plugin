...
async function saveTask(event){
  ...
  }catch(error){if(saved){try{await load({quiet:true,required:true});message(id?"Task updated after refresh retry":"Task created after refresh retry");}catch{state.needsRefresh=true;message(`Task saved, but refresh failed: ${error.message}`,true);}}else if(error.status===409){try{await load({quiet:true,required:true});dialog.close();message("Save conflict; board reloaded.",true);}catch{state.needsRefresh=true;message(`Save conflict and board refresh failed: ${error.message}`,true);}}else if(error.status)message(`Save failed: ${error.message}`,true);else{try{await load({quiet:true,required:true});dialog.close();message("Save response lost; board reconciled. Check the task before retrying.",true);}catch{state.needsRefresh=true;message(`Save response lost and board refresh failed: ${error.message}`,true);}}}finally{if(!state.needsRefresh)setDialogSaving(false);if(!state.needsRefresh)state.moving=false;render();}
}
