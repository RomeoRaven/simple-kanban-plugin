...
async function saveTask(event){
  ...
  try{
    if(id){const current=taskById(id);const desiredStatus=payload.status;delete payload.status;payload.expected_version=Number(document.getElementById("task-version").value);const updated=await request(`/tasks/${encodeURIComponent(id)}`,{method:"PATCH",body:JSON.stringify(payload)});if(current.status!==desiredStatus)await request(`/tasks/${encodeURIComponent(id)}/move`,{method:"POST",body:JSON.stringify({destination_status:desiredStatus,before_id:null,expected_version:updated.task.version})});}
